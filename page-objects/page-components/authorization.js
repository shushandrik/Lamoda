class Autorization {
  constructor(page) {
    ...
    this.favoritsLinkAfterAuthorization = page.locator("a[class='router-link-active router-link-exact-active _root_clp6c_2 _secondaryLabel_clp6c_13 _link_12pcw_8 _link_12pcw_8']")
  ...
  async quickVkloginButton() {
    await this.loginButton.click();
    // await this.page.waitForSelector(this.quickAuthVk)
  ...
  async registrationNewUser(name, email, phoneNumber9digit, password) {
    ...
    await this.page.keyboard.press('Tab');
    await this.page.keyboard.type(password);   
    //await this.policyAgreementCheckbox.check();
    await this.page.check(this.policyAgreementCheckbox);
    //await this.page.uncheck(this.subscribeNewsCheckbox);
  }
}
